  ...
  describe "POST #create" do
    context "with valid params" do
      ...
      it "creates the users as an association" do
        ...
        expect {
          post :create, group: attributes_for(:group), users: users.map { |u| u.user_id }
      ...
      it "creates permissions as an association" do
        ...
        expect {
          post :create, group: attributes_for(:group), forums: forums.map { |f| f.forum_id }, permissions: %w(read read read)
        }.to change(ForumGroupPermission, :count).by(3)
      end
    end
  ...
  describe "PUT #update" do
    context "with valid params" do
      ...
      it "creates the users as an association" do
        users = create_list(:user, 3)
        expect {
          post :update, id: group.to_param, group: new_attributes, users: users.map { |u| u.user_id }
        }.to change(GroupUser, :count).by(3)
      end
      ...
      it "creates permissions as an association" do
        ...
        expect {
          post :update, id: group.to_param, group: new_attributes, forums: forums.map { |f| f.forum_id }, permissions: %w(read read read)
        }.to change(ForumGroupPermission, :count).by(3)
      end
    end
