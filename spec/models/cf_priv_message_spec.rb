# -*- coding: utf-8 -*-

require 'rails_helper'

RSpec.describe CfPrivMessage, type: :model do
  it "is valid with subject, body, sender_id, recipient_id and owner_id" do
    expect(CfPrivMessage.new(subject: "Regarding: blue prints of the death star",
                             body: "The blue prints of the death star revealed a security problem",
                             sender_id: 1,
                             recipient_id: 2,
                             owner_id: 1)).to be_valid
  end

  it "is invalid wo subject" do
    expect(CfPrivMessage.new(body: "The blue prints of the death star revealed a security problem",
                             sender_id: 1,
                             recipient_id: 2,
                             owner_id: 1)).to be_invalid
  end

  it "is invalid wo body" do
    expect(CfPrivMessage.new(subject: "Regarding: blue prints of the death star",
                             sender_id: 1,
                             recipient_id: 2,
                             owner_id: 1)).to be_invalid
  end

  it "is invalid wo sender_id" do
    expect(CfPrivMessage.new(subject: "Regarding: blue prints of the death star",
                             body: "The blue prints of the death star revealed a security problem",
                             recipient_id: 2,
                             owner_id: 1)).to be_invalid
  end

  it "is invalid wo recipient_id" do
    expect(CfPrivMessage.new(subject: "Regarding: blue prints of the death star",
                             body: "The blue prints of the death star revealed a security problem",
                             sender_id: 1,
                             owner_id: 1)).to be_invalid
  end

  it "is invalid wo owner_id" do
    expect(CfPrivMessage.new(subject: "Regarding: blue prints of the death star",
                             body: "The blue prints of the death star revealed a security problem",
                             sender_id: 1,
                             recipient_id: 2)).to be_invalid
  end

  it "returns the other id for partner" do
    pm = CfPrivMessage.new(subject: "Regarding: blue prints of the death star",
                           body: "The blue prints of the death star revealed a security problem",
                           sender_id: 1,
                           recipient_id: 2,
                           owner_id: 1)
    expect(pm.partner_id(build(:cf_user, user_id: 1))).to eq 2
    expect(pm.partner_id(build(:cf_user, user_id: 2))).to eq 1
  end
end

# eof
