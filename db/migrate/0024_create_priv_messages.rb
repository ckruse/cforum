...
CREATE TABLE priv_messages (
  ...
  sender_id BIGINT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE ON UPDATE CASCADE,
  recipient_id BIGINT REFERENCES users(user_id) ON DELETE CASCADE ON UPDATE CASCADE,
